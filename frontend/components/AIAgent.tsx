...
interface AIAgentProps {
  ...
  isLoading: boolean;
  queueStatus?: { queuePosition?: number; estimatedWaitSeconds?: number; code?: string };
}
...
const AIAgent: React.FC<AIAgentProps> = ({ isOpen, onClose, messages, onSendMessage, isLoading, queueStatus }) => {
  ...
  // Debounced send function (500ms to prevent rapid requests)
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-4">
          {messages.map((msg, index) => (
            <div key={index}>
              <div className={`flex items-end gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'model' && <div className="w-8 h-8 bg-slate-200 rounded-full flex items-center justify-center flex-shrink-0"><Icon name="bot" className="w-5 h-5 text-slate-500" /></div>}
          ...
          {isLoading && (
            <div className="flex items-end gap-2 justify-start">
              <div className="w-8 h-8 bg-slate-200 rounded-full flex items-center justify-center flex-shrink-0"><Icon name="bot" className="w-5 h-5 text-slate-500" /></div>
        ...
        <div className="p-4 border-t border-gray-200">
          <div className="relative">
            <textarea
              ...
              rows={1}
              disabled={isLoading}
              className="w-full pl-4 pr-12 py-2 border border-gray-300 rounded-full resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
            />
